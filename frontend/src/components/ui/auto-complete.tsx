...
function Autocomplete({
  ...
  const scrollIntoView = (direction: "up" | "down") => {
    switch (direction) {
      case "up": {
        const diffUp = activeIndex === 0 ? filteredArray.length - 1 : -1;
        itemsRef.current[activeIndex + diffUp]?.scrollIntoView({
          behavior: "auto",
          block: "nearest",
          inline: "nearest",
        });
      ...
      case "down": {
        const diffDown =
          activeIndex === filteredArray.length - 1 ? -activeIndex : 1;
        itemsRef.current[activeIndex + diffDown]?.scrollIntoView({
          behavior: "auto",
          block: "nearest",
          inline: "nearest",
        });
  ...
  const handleArrowKeys = (e: React.KeyboardEvent<HTMLInputElement>) => {
    switch (e.key) {
      case "ArrowUp":
        e.preventDefault();
        setActiveIndex(
          activeIndex === 0 ? filteredArray.length - 1 : activeIndex - 1,
        ); // If the index goes below 0, send to the bottom
        scrollIntoView("up");
        break;
      case "ArrowDown":
        e.preventDefault();
        setActiveIndex(
          activeIndex === filteredArray.length - 1 ? 0 : activeIndex + 1,
        ); // If the index goes above the length of the array, send to top
        scrollIntoView("down");
        break;
      case "Enter":
        e.preventDefault();
    ...
    <>
      ...
      >
        ...
        <input
          ...
          type="text"
          value={searchValue}
          className="absolute flex items-center w-full h-full inset-x-0 inset-y-0 px-[12px] outline-none ring-major bg-transparent placeholder:text-muted z-50"
          tabIndex={0}
          onClick={() => {
            setIsOpen(!isOpen);
            if (isOpen) {
        ...
        {/* Carret */}
        <div className="absolute w-[2rem] right-0 inset-y-0 flex items-center justify-center">
          <svg
        ...
        {options ? (
          <div
            className={`${isOpen ? "block" : "hidden"} absolute top-10 right-0 left-0 bg-black/50 text-muted cursor-pointer ring-1 ring-major rounded-sm max-h-[20rem] overflow-y-scroll z-50`}
          >
