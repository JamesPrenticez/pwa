      ...
      (value) => {
        ...
        if (atIndex === -1 || atIndex === value.length - 1) {
          return {
            ...
            errorMessage: errorMessage
              ? errorMessage
              : 'Email must contain a domain name after "@"',
          };
        }
        ...
        if (!domainPart.includes(".") || domainPart.endsWith(".")) {
          return {
            ...
            errorMessage: errorMessage
              ? errorMessage
              : 'Email must contain a "." after "@" with a valid domain',
          };
        }
...
function createValidationSchema(
  schemaDefinition: IValidationSchema,
): IValidationSchema {
...
function resolver(
  ...
  schema: Record<string, FormValidator>,
...
export { formValidator as v, createValidationSchema, resolver };
