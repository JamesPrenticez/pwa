...
export function useForm<T>({
  ...
  const initialErrors = getInitalErrors(
    initialState,
    validationSchema,
    validatorFn,
  );
  ...
  const setFieldValue = (fieldKey: keyof T, newValue: T[keyof T]) => {
    ...
    const mergedValidationResult = {
      ...formErrors,
      [fieldKey as string]: validationResult[fieldKey as string],
    };
...
function getInitalErrors(
  initialState: any,
  validationSchema: any,
  validatorFn: any,
) {
  const initialErrors = Object.keys(initialState).reduce(
    (acc, key) => {
      ...
      if (value !== null && value !== undefined && value !== "") {
        ...
        const fieldValidationResult = validatorFn(
          { [key]: value },
